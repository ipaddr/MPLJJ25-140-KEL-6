...
import 'package:project_akhir/screens/auth/admin_register_screen.dart';
...
class _AdminLoginScreenState extends State<AdminLoginScreen> {
  ...
  Future<void> _login() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (success && mounted) {
        // Check if user is regular user, if yes, logout and show error
        if (!authProvider.isAdmin) {
          await authProvider.logout();
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Akun ini tidak terdaftar sebagai admin. Silakan login melalui halaman user.'),
